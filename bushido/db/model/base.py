...
class Entry(Base):
    ...
    label: Mapped[Optional[str]] = mapped_column(
        String(120)
    )  # free label or emoji alias
    note: Mapped[Optional[str]] = mapped_column(Text)

    workout: Mapped[Workout] = relationship(back_populates="entries")
    weights: Mapped[Optional[WeightsEntry]] = relationship(
        back_populates="entry", uselist=False, cascade="all, delete-orphan"
    )
    cardio: Mapped[Optional[CardioEntry]] = relationship(
        back_populates="entry", uselist=False, cascade="all, delete-orphan"
    )
    martial: Mapped[Optional[MartialEntry]] = relationship(
        back_populates="entry", uselist=False, cascade="all, delete-orphan"
    )
    stat: Mapped[Optional[BodyStatEntry]] = relationship(
        back_populates="entry", uselist=False, cascade="all, delete-orphan"
    )
    study: Mapped[Optional[StudyEntry]] = relationship(
        back_populates="entry", uselist=False, cascade="all, delete-orphan"
    )
...
class WeightsEntry(Base):
    ...
    entry_id: Mapped[int] = mapped_column(
        ForeignKey("entry.id", ondelete="CASCADE"), unique=True
    )
    ...
    sets: Mapped[list[WeightsSet]] = relationship(
        back_populates="weights", cascade="all, delete-orphan"
    )
...
class WeightsSet(Base):
    ...
    weights_id: Mapped[int] = mapped_column(
        ForeignKey("weights_entry.id", ondelete="CASCADE"), index=True
    )
    kg: Mapped[float] = mapped_column(Float)
    reps: Mapped[int] = mapped_column(Integer)
    rest: Mapped[Optional[int]] = mapped_column(Integer)  # seconds
    __table_args__ = (
        UniqueConstraint("weights_id", "id", name="uq_weightsset_identity"),
    )

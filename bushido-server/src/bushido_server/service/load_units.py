...
from bushidolib.contracts.unit import CardioUnit, GymUnit, LiftingUnit, WimhofUnit, LoadedUnits, CardioData, GymData, \
    LiftingData, WimhofData
...
def _(request: CardioLoadUnitRequest, session: Session) -> LoadedUnits:
    ...
    return [CardioUnit(name=unit.name, log_time=unit.log_time, comment=unit.comment, data=CardioData.model_validate(unit.data)) for unit in units]
...
def _(request: GymLoadUnitRequest, session: Session) -> LoadedUnits:
    ...
    units = repo.fetch_units(start_t=request.start_time, end_t=request.end_time)
    return [GymUnit(name=unit.name, log_time=unit.log_time, comment=unit.comment, data=GymData.model_validate(unit.data)) for unit in units]
...
def _(request: LiftingLoadUnitRequest, session: Session) -> LoadedUnits:
    ...
    units = repo.fetch_units(start_t=request.start_time, end_t=request.end_time)
    return [LiftingUnit(name=unit.name, log_time=unit.log_time, comment=unit.comment, data=LiftingData.model_validate(unit.data)) for unit in units]
...
def _(request: WimhofLoadUnitRequest, session: Session) -> LoadedUnits:
    ...
    units = repo.fetch_units(start_t=request.start_time, end_t=request.end_time)
    return [WimhofUnit(name=unit.name, log_time=unit.log_time, comment=unit.comment, data=WimhofData.model_validate(unit.data)) for unit in units]

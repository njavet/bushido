...
class EmojiHandler(te.BaseHandler):
    ...
    async def emoji_handler(self, update: telegram.Update, context: te.CallbackContext):
        ...
        res = self.um.process_string(
            input_string=context.args, user_id=user_id, recv_time=recv_time
        )
        if res.success:
            ans = "Unit confirmed!"
        else:
    ...
    async def handle_update(self, update, dispatcher, check_result, context=None):
        ...
        try:
            ...
            pass
        if key.decode() not in self.um.emoji2proc.keys():
    ...
    def set_local_time(message_date):
        ...
        recv_time = datetime.datetime(
            lt0.year, lt0.month, lt0.day, lt0.hour, lt0.minute, lt0.second
        )
        return recv_time

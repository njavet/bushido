...
class TimeTable(ModalScreen):
    BINDINGS = [("q", "app.pop_screen", "Back"), ("t", "toggle_tree", "Toggle Tree")]
    ...
    def collect_units(self):
        sq0 = (
            Unit.select(Unit.log_date, Unit.unit_emoji, Balance.weight)
            .join(Balance)
        ...
        sq1 = (
            Unit.select(Unit.log_date, Unit.unit_emoji)
            .join(Wimhof)
            .group_by(Unit.log_date)
            .alias("wimhof_units")
        )
...
def construct_table(year, dix):
    ...
    first = utilities.find_previous_sunday(datetime.date(year, 1, 1))
    if datetime.datetime.now().year == year:
        last = utilities.find_next_saturday(datetime.date.today())
    else:
        last = utilities.find_next_saturday(datetime.date(year, 12, 31))

    days = (last - first).days
    while last >= first:
        dt = datetime.datetime.strftime(last, "%d.%m.%y")
        table.add_row(str(days), dt, str(dd[last]))

...
class TgCom:
    def __init__(self, um):
        ...
        self.tg_agent = TelegramClient(self.agent_session, self.api_id, self.api_hash)
        self.tg_bot = self.setup_bot()

    def setup_bot(self):
        tg_bot = TelegramClient(self.t800_session, self.api_id, self.api_hash)
        tg_bot.add_event_handler(self.msg_recv_handler, NewMessage(incoming=True))
        return tg_bot
    ...
    async def msg_recv_handler(self, event):
        ...
        ans = self.um.log_unit(event.message.date.timestamp(), event.message.message)
